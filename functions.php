...
function bsktv_search_posts($q){if(is_admin()||!$q->is_main_query())return;if($q->is_search()){$q->set('post_type','post');$q->set('post_status','publish');$q->set('orderby','modified');$q->set('order','DESC');$q->set('meta_query',array('relation'=>'OR',array('key'=>'bsktv_status','compare'=>'NOT EXISTS'),array('key'=>'bsktv_status','value'=>array('active','pending','paused'),'compare'=>'IN')));}elseif($q->is_home()||$q->is_archive()||$q->is_tax()){$q->set('post_type','post');$q->set('post_status','publish');if(!$q->get('orderby')){$q->set('orderby','modified');$q->set('order','DESC');}if($q->is_home())$q->set('meta_query',array('relation'=>'OR',array('key'=>'bsktv_status','compare'=>'NOT EXISTS'),array('key'=>'bsktv_status','value'=>array('active','pending','paused'),'compare'=>'IN')));}}add_action('pre_get_posts','bsktv_search_posts');
function bsktv_excerpt_length(){return 24;} add_filter('excerpt_length','bsktv_excerpt_length');
function bsktv_customize($wp_customize){$wp_customize->add_section('bsktv_home',array('title'=>'BSKTV 首頁設定','priority'=>30));$wp_customize->add_setting('bsktv_hero_title',array('default'=>'探索台灣商務 KTV 與酒店資訊','sanitize_callback'=>'sanitize_text_field'));$wp_customize->add_control('bsktv_hero_title',array('label'=>'首頁主標題','section'=>'bsktv_home','type'=>'text'));$wp_customize->add_setting('bsktv_hero_text',array('default'=>'以店家為核心，快速查找城市、類型、特色與完整店家介紹。','sanitize_callback'=>'sanitize_textarea_field'));$wp_customize->add_control('bsktv_hero_text',array('label'=>'首頁說明','section'=>'bsktv_home','type'=>'textarea'));$wp_customize->add_setting('bsktv_line_url',array('default'=>'','sanitize_callback'=>'esc_url_raw'));$wp_customize->add_control('bsktv_line_url',array('label'=>'聯絡幹部 LINE 加好友連結','description'=>'貼上你的 LINE 加好友網址，例如 https://lin.ee/xxxxx。若個別店家未設定專屬 LINE，店家頁會使用此全站連結。','section'=>'bsktv_home','type'=>'url'));}
