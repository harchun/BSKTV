...
function bsktv_search_posts($q){if(is_admin()||!$q->is_main_query())return;if($q->is_search()){$q->set('post_type','post');$q->set('post_status','publish');$q->set('orderby','modified');$q->set('order','DESC');}elseif($q->is_home()||$q->is_archive()||$q->is_tax()){$q->set('post_type','post');$q->set('post_status','publish');if(!$q->get('orderby')){$q->set('orderby','modified');$q->set('order','DESC');}}}add_action('pre_get_posts','bsktv_search_posts');
