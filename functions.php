...
function bsktv_front_store_query($q){$q->set('post_type','post');$q->set('post_status','publish');$q->set('meta_query',array('relation'=>'OR',array('key'=>'bsktv_status','compare'=>'NOT EXISTS'),array('key'=>'bsktv_status','value'=>array('active','pending','paused'),'compare'=>'IN')));}
function bsktv_search_posts($q){if(is_admin()||!$q->is_main_query())return;if($q->is_search()){$q->set('orderby','modified');$q->set('order','DESC');bsktv_front_store_query($q);}elseif($q->is_home()||$q->is_archive()||$q->is_tax()){bsktv_front_store_query($q);if(!$q->get('orderby')){$q->set('orderby','modified');$q->set('order','DESC');}}}add_action('pre_get_posts','bsktv_search_posts');
